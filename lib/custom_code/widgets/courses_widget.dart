    ...
    return Padding(
      ...
      child: ElevatedButton.icon(
        onPressed: () async {
          ...
          final message =
              'Check out this amazing course: $courseTitle!\n\nThe first lesson is FREE!\n\nUse my coupon code $couponCode to get ₹50 off when you decide to purchase.\n\n$courseLink';
...
class _AdminPanelPageState extends State<AdminPanelPage> {
  ...
  final String adminPin = '944797'; // Change this to your desired PIN

  @override
  void initState() {
    super.initState();
    // Show authentication dialog immediately
    WidgetsBinding.instance.addPostFrameCallback((_) {
      _showPasswordDialog();
    });
  }

  // Show simple password authentication dialog
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          TextButton(
            onPressed: () {
              Navigator.of(context).pop();
              Navigator.of(context).pop(); // Exit admin page
            },
            child: Text(
              'Cancel',
  ...
  Future<void> _loadPendingRequests() async {
    ...
    try {
      ...
      final response = await supabase.from('user_course_access').select('''
        id, 
        has_paid,
        user_id,
        course_id,
      ...
      if (mounted) {
        safeSetState(() {
          ...
          pendingUsers = List<Map<String, dynamic>>.from(response);
          isLoading = false;
        });
      }
    } catch (e) {
  ...
  Future<void> _grantAccess(String accessId) async {
    try {
      await supabase
          .from('user_course_access')
  ...
  @override
  Widget build(BuildContext context) {
    // If not authenticated, just show a simple login page
    if (!isAuthenticated) {
      return Scaffold(
        appBar: AppBar(
      ...
      drawer: Drawer(
        child: ListView(
          ...
          children: [
            ...
            ListTile(
              ...
              onTap: () {
                ...
                Navigator.push(
                  context,
                  MaterialPageRoute(
                      builder: (context) => const HomeDataUpdatePage()),
                );
              },
            ),
            // ListTile(
            ...
            ListTile(
              leading: const Icon(Icons.people, color: Colors.black),
              title: const Text('Course Access'),
              onTap: () {
                Navigator.pop(context);
                Navigator.push(
                  context,
                  MaterialPageRoute(
                      builder: (context) => const CustomAccessAdminPage()),
                );
              },
            ),
            // ListTile(
            //   leading: const Icon(Icons.people, color: Colors.black),
            //   title: const Text('Shares add'),
            //   onTap: () {
            //     Navigator.pop(context);
            //     Navigator.push(
            //       context,
            //       MaterialPageRoute(
            //           builder: (context) => const AdminShareMarket()),
            //     );
            //   },
            // ),
            // ListTile(
            //   leading: const Icon(Icons.people, color: Colors.black),
            //   title: const Text('Total Shares add'),
            //   onTap: () {
            //     Navigator.pop(context);
            //     Navigator.push(
            //       context,
            //       MaterialPageRoute(
            //           builder: (context) => const TotalSharesPage()),
            //     );
            //   },
            // ),
            // ListTile(
            ...
            //     Navigator.push(
            //       context,
            //       MaterialPageRoute(builder: (context) => const StockUpload()),
            //     );
            //   },
            // ),
            // ListTile(
            ...
            //     Navigator.push(
            //       context,
            //       MaterialPageRoute(builder: (context) => const PromptUpload()),
            //     );
            //   },
            // ),
            // ListTile(
            ...
            //       context,
            //       MaterialPageRoute(
            //           builder: (context) => const AdminEphemeralMessagesPage()),
            //     );
            //   },
            // ),
            // ListTile(
            //   leading: const Icon(Icons.people, color: Colors.black),
            //   title: const Text('status Storage'),
            //   onTap: () {
            //     Navigator.pop(context);
            //     Navigator.push(
            //       context,
            //       MaterialPageRoute(
            //           builder: (context) => const StatusAdminPanel()),
            //     );
            //   },
            // ),
            ListTile(
            ...
            : pendingUsers.isEmpty
                ...
                : ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      final userEmail = item['users']['email'] ?? 'No email';
                      ...
                      final userName = () {
                        try {
                          ...
                          if (user != null) {
                            final profiles =
                                user['profile'] as List<dynamic>?; // ✅
                            if (profiles != null && profiles.isNotEmpty) {
                              final profile =
                                  profiles.first as Map<String, dynamic>;
                              return profile['name'] ?? 'noname';
                            }
                          }
                          return 'noname';
                        } catch (e) {
                      ...
                      final userPhone = () {
                        try {
                          ...
                          if (user != null) {
                            final profiles = user['profile'] as List<dynamic>?;
                            if (profiles != null && profiles.isNotEmpty) {
                              final profile =
                                  profiles.first as Map<String, dynamic>;
                              return profile['phone_no'] as String? ?? '';
                            }
                          }
                          return '';
                        } catch (e) {
                      ...
                      final accessId = item['id'];
                      final createdAt = item['created_at'];

                      final requestTime = DateTime.parse(createdAt);
                      final now = DateTime.now();
                      final pendingDuration = now.difference(requestTime);
                      final isRecent = pendingDuration.inHours < 24;
                      ...
                      return Card(
                        ...
                        shape: RoundedRectangleBorder(
                          ...
                          side: BorderSide(
                            color:
                                isRecent ? Colors.red : Colors.yellow.shade700,
                            ...
                            children: [
                              ...
                              Row(
                                ...
                                children: [
                                  OutlinedButton.icon(
                                    onPressed: () {
                                      if (userPhone.isNotEmpty &&
                                        ...
                                        final cleanPhone = userPhone
                                            .replaceAll(' ', '')
                                            .replaceAll('-', '');
                                        final phoneUrl = "tel:$cleanPhone";
                                        launchUrl(Uri.parse(phoneUrl));
                                      } else {
                                        // Fallback to WhatsApp if no valid phone number
                                        final whatsappUrl =
                                            "https://wa.me/?text=Regarding your course access request for: $courseTitle";
                                  ...
                                  ElevatedButton.icon(
                                    onPressed: () => _grantAccess(accessId),
                                    style: ElevatedButton.styleFrom(
                                      backgroundColor: Colors.yellow.shade700,
                                    ),
...
class CustomAccessAdminPage extends StatefulWidget {
  const CustomAccessAdminPage({super.key});

  @override
  _CustomAccessAdminPageState createState() => _CustomAccessAdminPageState();
}

class _CustomAccessAdminPageState extends State<CustomAccessAdminPage> {
  final supabase = SupaFlow.client;
  ...
  bool isAddingAccess = false;
  String? selectedCoupon;

  // Available coupons - you can modify this list
  ...
  Future<void> _loadAllCourses() async {
    try {
      ...
      final response = await supabase.from('allcourses').select('''
        course_id,
        course_title,
        course_description,
        course_price,
      ...
      if (mounted) {
        // Remove duplicates based on course title
        final uniqueTitles = <String>{};
        final uniqueCourses = <Map<String, dynamic>>[];
  ...
  Future<void> _searchUsers(String query) async {
    ...
    try {
      ...
      final emailResponse = await supabase.from('users').select('''
            id,
            email,
            created_at,
            profile (
          ...
          ''').ilike('email', '%$query%').limit(20);

      // Search by name in profile
      final nameResponse = await supabase.from('users').select('''
            id,
            email,
            created_at,
            profile!inner (
          ...
          ''').ilike('profile.name', '%$query%').limit(20);

      if (mounted) {
        // Combine and deduplicate results
        final combinedUsers = <String, Map<String, dynamic>>{};

        for (final user in emailResponse) {
          combinedUsers[user['id']] = user;
        }

        for (final user in nameResponse) {
          combinedUsers[user['id']] = user;
        }

        setState(() {
          searchedUsers = combinedUsers.values.toList();
  ...
  Future<void> _searchCourses(String query) async {
    ...
    try {
      ...
      final response = await supabase.from('allcourses').select('''
           ...
           course_price,
           course_created_at
          ''').ilike('course_title', '%$query%').limit(50);
  ...
  // Add custom access
  Future<void> _addCustomAccess() async {
    if (selectedUser == null || selectedCourse == null) {
      _showErrorSnackBar('Please select both user and course');
      return;
    }
    ...
    try {
      ...
      // Get the user ID and course ID safely
      final userId = selectedUser!['id']?.toString();
      final courseId = selectedCourse!['course_id']?.toString();

      if (userId == null || courseId == null) {
        _showErrorSnackBar('Invalid user or course selection');
        return;
      }

      print('Checking access for user: $userId, course: $courseId');
      ...
      print('Existing access check result: $existingAccess');

      if (existingAccess.isNotEmpty) {
        final access = existingAccess.first;
        final hasPaid = access['has_paid'];

        if (hasPaid == true) {
          _showErrorSnackBar('User already has paid access to this course');
          return;
        } else {
          // Prepare update data
          ...
          final couponValue = selectedCoupon?.isNotEmpty == true
              ? selectedCoupon
          ...
          if (couponValue != null) {
            updateData['applied_coupon'] = couponValue;
          }
          ...
          _showSuccessSnackBar('Updated existing access request');
        }
      } else {
        // Prepare insert data
        ...
        if (couponValue != null) {
          insertData['applied_coupon'] = couponValue;
        }

        print('Inserting with data: $insertData');

        // Insert new record
        await supabase.from('user_course_access').insert(insertData);

        _showSuccessSnackBar('Custom access added successfully');
      }

      // Reset form
      _resetForm();
    } catch (e) {
      print('Error adding custom access: $e');
      print('Stack trace: ${StackTrace.current}');
      _showErrorSnackBar('Error adding access: ${e.toString()}');
    } finally {
      if (mounted) {
        setState(() {
          ...
          children: [
            ...
            TextField(
              ...
              decoration: InputDecoration(
                ...
                labelStyle: TextStyle(color: Colors.yellow.shade200),
                prefixIcon: Icon(Icons.search, color: Colors.yellow.shade200),
                enabledBorder: OutlineInputBorder(
                  borderSide: BorderSide(color: Colors.yellow.shade700),
                ),
                focusedBorder: OutlineInputBorder(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          if (selectedUser!['profile'] != null &&
                            ...
                            Text(
                              selectedUser!['profile'][0]['name'] ?? 'No name',
            ...
            if (searchedUsers.isNotEmpty && selectedUser == null) ...[
              const SizedBox(height: 12),
              Container(
                height: 200,
                decoration: BoxDecoration(
  ...
  Widget _buildCourseSearchSection() {
    return Card(
      color: Colors.black87,
      child: Padding(
        padding: const EdgeInsets.all(16),
        child: Column(
          ...
          children: [
            ...
            TextField(
              ...
              style: TextStyle(color: Colors.yellow.shade100),
              decoration: InputDecoration(
                labelText: 'Search courses by title',
                labelStyle: TextStyle(color: Colors.yellow.shade200),
                prefixIcon: Icon(Icons.search, color: Colors.yellow.shade200),
                enabledBorder: OutlineInputBorder(
                  borderSide: BorderSide(color: Colors.yellow.shade700),
            ...
            if (searchedCourses.isNotEmpty && selectedCourse == null) ...[
              const SizedBox(height: 12),
              Container(
                height: 200,
                decoration: BoxDecoration(
        ...
        child: Column(
          ...
          children: [
            ...
            DropdownButtonFormField<String>(
              ...
              style: TextStyle(color: Colors.yellow.shade100),
              decoration: InputDecoration(
                labelText: 'Select predefined coupon',
                labelStyle: TextStyle(color: Colors.yellow.shade200),
                enabledBorder: OutlineInputBorder(
                  borderSide: BorderSide(color: Colors.yellow.shade700),
                ),
              ...
              items: [
                ...
                ...availableCoupons.map((coupon) => DropdownMenuItem<String>(
                      value: coupon,
                      child: Text(
                        coupon,
                        style: TextStyle(color: Colors.yellow.shade100),
            ...
            TextField(
              ...
              style: TextStyle(color: Colors.yellow.shade100),
              decoration: InputDecoration(
                labelText: 'Or enter custom coupon',
                labelStyle: TextStyle(color: Colors.yellow.shade200),
                prefixIcon:
                    Icon(Icons.local_offer, color: Colors.yellow.shade200),
                enabledBorder: OutlineInputBorder(
                  borderSide: BorderSide(color: Colors.yellow.shade700),
                ),
                focusedBorder: OutlineInputBorder(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Add Custom Course Access'),
        backgroundColor: Colors.black87,
...
// Helper function to use in your app navigation
...
// Function to create user course access record (kept as is)
Future<void> createUserCourseAccessRecord(
  ...
  try {
    ...
    // If no record exists, create one with has_paid = false
    if (existing == null) {
      await supabase.from('user_course_access').insert({
        'user_id': userId,
        'course_id': courseId,
      ...
      print('Created new access request for user $userId and course $courseId');
    } else {
      print(
          'Access request already exists for user $userId and course $courseId');
    }
  } catch (e) {
    print('Error creating/checking user course access record: $e');
    rethrow;
  }
}
...
class _UserManagementPageState extends State<UserManagementPage> {
  ...
  List<Map<String, dynamic>> allUsers =
      []; // Store all loaded users for local operations
  bool isLoading = false;
  ...
  void _onSearchChanged() {
    ...
    _searchTimer = Timer(const Duration(milliseconds: 500), () {
      if (query.isEmpty) {
        // If search is cleared, show original paginated data
        safeSetState(() {
  ...
  Future<void> _performServerSearch(String query) async {
    ...
    try {
      safeSetState(() => isSearching = true);

      // Search by email
      final emailResults = await supabase.from('users').select('''
          id,
          email,
          created_at,
          followers,
          password,
          profile!inner(name, shop_name)
        ''').ilike('email', '%$query%');

      // Search by profile name
      final nameResults = await supabase.from('users').select('''
          id,
          email,
          created_at,
          followers,
      ...
      // Search by shop name
      final shopResults = await supabase.from('users').select('''
          id,
          email,
          created_at,
          followers,
          password,
          profile!inner(name, shop_name)
        ''').ilike('profile.shop_name', '%$query%');

      // Combine and deduplicate results
      final Set<String> seenIds = {};
      final List<Map<String, dynamic>> combinedResults = [];

      for (final result in [...emailResults, ...nameResults, ...shopResults]) {
      ...
      combinedResults.sort((a, b) => DateTime.parse(b['created_at'])
          .compareTo(DateTime.parse(a['created_at'])));

      safeSetState(() {
        filteredUsers = combinedResults;
        isSearching = false;
      });
    } catch (e) {
      print('Error searching users: $e');
      safeSetState(() => isSearching = false);
  ...
  Future<void> fetchUsers({bool isRefresh = false}) async {
    try {
      safeSetState(() {
        isLoading = isRefresh || users.isEmpty;
        if (isRefresh) {
          currentPage = 0;
          hasMore = true;
        }
      ...
      safeSetState(() {
        if (isRefresh) {
          ...
          users.addAll(List<Map<String, dynamic>>.from(response));
          allUsers.addAll(List<Map<String, dynamic>>.from(response));
        }

        hasMore = response.length == pageSize;
        isLoading = false;

        // Only show loaded data if not searching
  ...
  Future<void> _loadMoreUsers() async {
    ...
    try {
      safeSetState(() => isLoadingMore = true);
      currentPage++;
      ...
      safeSetState(() {
        ...
        // Only update filtered users if not searching
        if (searchQuery.isEmpty) {
          filteredUsers = List.from(allUsers);
        }
      });
    } catch (e) {
      safeSetState(() => isLoadingMore = false);
      _showSnackBar('Error loading more users: $e', Colors.red);
    }
  }
  ...
  Future<void> _updateFollowers(
      Map<String, dynamic> user, String newFollowersText) async {
    final newFollowers = int.tryParse(newFollowersText);
    ...
    try {
      // Show loading
      _showSnackBar('Updating followers...', Colors.blue);

      // Update in database
      await supabase
          .from('users')
          .update({'followers': newFollowers}).eq('id', user['id']);

      // Update local data
      safeSetState(() {
        // Update in all lists
        _updateUserInLists(user['id'], 'followers', newFollowers);
      });

      _showSnackBar(
        'Followers updated to $newFollowers successfully!',
        Colors.green,
      );
    } catch (e) {
      print('Error updating followers: $e');
      _showSnackBar('Error updating followers: $e', Colors.red);
    }
  }

  void _updateUserInLists(dynamic userId, String field, dynamic value) {
    ...
    for (int i = 0; i < allUsers.length; i++) {
      if (allUsers[i]['id'] == userId) {
        allUsers[i][field] = value;
        break;
      }
    }

    // Update in filteredUsers list
    for (int i = 0; i < filteredUsers.length; i++) {
      if (filteredUsers[i]['id'] == userId) {
        filteredUsers[i][field] = value;
        break;
      }
    }
  }

  void _showUpdateFollowersDialog(
      Map<String, dynamic> user, int currentFollowers) {
    final TextEditingController followersController = TextEditingController(
      text: currentFollowers.toString(),
    );
          ...
          content: Column(
            ...
            children: [
              ...
              TextField(
                ...
                autofocus: true,
                onSubmitted: (value) {
                  _updateFollowers(user, followersController.text);
                  Navigator.of(context).pop();
                },
              ),
            ],
  ...
  Widget _buildSearchBar() {
    return Padding(
      ...
      child: TextField(
        ...
        decoration: InputDecoration(
          hintText:
              'Search users by name, shop_name, email, date, or followers...',
  ...
  void _copyToClipboard(String text, String type) {
    ...
    _showSnackBar('$type copied to clipboard', Colors.green);
  }

  Widget _buildUserCard(Map<String, dynamic> user) {
    // Handle profile data - it might be a List or Map
    Map<String, dynamic>? profile;
    final profileData = user['profile'];

    if (profileData is List && profileData.isNotEmpty) {
      profile = profileData[0] as Map<String, dynamic>?;
    } else if (profileData is Map<String, dynamic>) {
      profile = profileData;
    }

    final name = profile?['name']?.toString() ?? 'N/A';
    final shopname = profile?['shop_name']?.toString() ?? 'N/A';
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              children: [
                const Icon(Icons.store, size: 20, color: Colors.amber),
                const SizedBox(width: 8),
                Expanded(
                  child: Text(
                    shopname,
            ...
            Row(
              children: [
                ...
                // Followers with tap to edit
                GestureDetector(
                  onTap: () => _showUpdateFollowersDialog(
                    user,
                    int.tryParse(user['followers']?.toString() ?? '0') ?? 0,
                  ),
                  child: Container(
                    padding:
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        const SizedBox(width: 4),
                        const Icon(Icons.edit, size: 12, color: Colors.purple),
                      ],
                    ),
                  ),
                ),
...
class HomeDataUpdatePage extends StatefulWidget {
  const HomeDataUpdatePage({Key? key}) : super(key: key);

  @override
  State<HomeDataUpdatePage> createState() => _HomeDataUpdatePageState();
}

class _HomeDataUpdatePageState extends State<HomeDataUpdatePage> {
  final supabase = Supabase.instance.client;

  // Text controllers
  final TextEditingController _textRow1Controller = TextEditingController();
  final TextEditingController _textRow2Controller = TextEditingController();
  final TextEditingController _textRow3Controller = TextEditingController();
  ...
  @override
  void dispose() {
    _textRow1Controller.dispose();
    _textRow2Controller.dispose();
    _textRow3Controller.dispose();
    super.dispose();
  }
  ...
  Future<void> _loadCurrentData() async {
    ...
    try {
      ...
      if (response.isNotEmpty) {
        ...
        currentRecordId = data['id']?.toString();

        setState(() {
          _textRow1Controller.text = data['text_row_1'] ?? 'Connect';
          _textRow2Controller.text = data['text_row_2'] ?? 'Showcase';
          _textRow3Controller.text = data['text_row_3'] ?? 'Worldwide';
        });
      } else {
        // Set default values if no data exists
        setState(() {
          _textRow1Controller.text = 'Connect';
          _textRow2Controller.text = 'Showcase';
          _textRow3Controller.text = 'Worldwide';
        });
      }
    } catch (e) {
  ...
  Future<void> _updateHomeData() async {
    if (_textRow1Controller.text.trim().isEmpty ||
        _textRow2Controller.text.trim().isEmpty ||
        _textRow3Controller.text.trim().isEmpty) {
    ...
    try {
      final dataToUpdate = {
        'text_row_1': _textRow1Controller.text.trim(),
        'text_row_2': _textRow2Controller.text.trim(),
        'text_row_3': _textRow3Controller.text.trim(),
        'created_at': DateTime.now().toIso8601String(),
      };
      ...
      if (currentRecordId != null) {
        ...
        // Insert new record
        dataToUpdate['created_at'] = DateTime.now().toIso8601String();
        final response =
            await supabase.from('home').insert(dataToUpdate).select().single();
        currentRecordId = response['id']?.toString();
      }

      _showSuccessSnackBar('Home data updated successfully!');

      // Navigate back to home or refresh home page
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Update Home Data'),
        elevation: 0,
      ),
      body: isLoading
          ...
          : Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  // Text Row 1
                  TextField(
                    controller: _textRow1Controller,
                    decoration: const InputDecoration(
                      labelText: 'Text Row 1',
                      hintText: 'Enter first line text',
                      border: OutlineInputBorder(),
                    ),
                  ...
                  // Text Row 2
                  TextField(
                    controller: _textRow2Controller,
                    decoration: const InputDecoration(
                      labelText: 'Text Row 2',
                      hintText: 'Enter second line text',
                      border: OutlineInputBorder(),
                    ),
                    textCapitalization: TextCapitalization.words,
                  ),
                  ...
                  // Text Row 3
                  TextField(
                    controller: _textRow3Controller,
                    decoration: const InputDecoration(
                      labelText: 'Text Row 3',
                      hintText: 'Enter third line text',
                      border: OutlineInputBorder(),
                    ),
                    textCapitalization: TextCapitalization.words,
                  ),
                  ...
                  Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 8),
                        Text(
                          _textRow1Controller.text.isEmpty
                              ? 'Text Row 1'
                              : _textRow1Controller.text,
                          style: const TextStyle(
                              fontSize: 18, fontWeight: FontWeight.bold),
                        ),
                        Text(
                          _textRow2Controller.text.isEmpty
                              ? 'Text Row 2'
                              : _textRow2Controller.text,
                          style: const TextStyle(fontSize: 16),
                        ),
                        Text(
                          _textRow3Controller.text.isEmpty
                              ? 'Text Row 3'
                              : _textRow3Controller.text,
                          style: const TextStyle(fontSize: 16),
                        ),
                      ],
                    ),
                  ...
                  // Update Button
                  ElevatedButton(
                    onPressed: isUpdating ? null : _updateHomeData,
                    style: ElevatedButton.styleFrom(
                      padding: const EdgeInsets.symmetric(vertical: 16),
                      shape: RoundedRectangleBorder(
                        borderRadius: BorderRadius.circular(8),
                  ...
                  TextButton(
                    onPressed:
                        isLoading || isUpdating ? null : _loadCurrentData,
                    child: const Text('Refresh Current Data'),
                  ),
                ],
              ),
            ),
                      ...
                      children: [
                        ...
                        Row(
                          children: [
                            ...
                            Expanded(
                              child: Text(
                                'Your friends get ₹50 off with your code',
                                style: TextStyle(
                                  color: Colors.yellow.shade100,
                                  fontSize: 12,
                                ),
                        ...
                        Row(
                          children: [
                            ...
                            Expanded(
                              child: Text(
                                'You earn ₹300 when they purchase',
                                style: TextStyle(
                                  color: Colors.yellow.shade100,
                                  fontSize: 12,
                                ),
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: 4),
                          Text(
                            'Friends get ₹50 off, you get ₹300 when they purchase!',
                            style: TextStyle(
                              color: Colors.yellow.shade100,
                              fontSize: 12,
                            ),
