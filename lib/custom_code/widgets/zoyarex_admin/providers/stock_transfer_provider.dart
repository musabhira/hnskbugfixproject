...
final stockTransfersProvider = FutureProvider<List<StockTransferModel>>((ref) async {
  final response = await ZoyarexSupabase.client
      .from('gt_vouchers')
      .select('*')
      .applyTenantFilter('gt_vouchers')
      .eq('voucher_type', 'stockTransfer')
      .order('voucher_date', ascending: false);

  final data = response as List<dynamic>;
  return data.map((json) => StockTransferModel.fromJson(json)).toList();
});
