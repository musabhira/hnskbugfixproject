...
class PaymentInModel {
  ...
  factory PaymentInModel.fromJson(Map<String, dynamic> json) {
    return PaymentInModel(
      ...
      partyName: json['party_name']?.toString() ?? 'Customer',
...
final paymentInsProvider = FutureProvider<List<PaymentInModel>>((ref) async {
  final response = await ZoyarexSupabase.client
      .from('gt_vouchers')
      .select('*')
      .applyTenantFilter('gt_vouchers')
      .eq('voucher_type', 'paymentIn')
      .order('voucher_date', ascending: false);

  final data = response as List<dynamic>;
  return data.map((json) => PaymentInModel.fromJson(json)).toList();
});
