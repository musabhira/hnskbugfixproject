...
final stockAdjustmentsProvider = FutureProvider<List<StockAdjustmentModel>>((ref) async {
  final response = await ZoyarexSupabase.client
      .from('gt_vouchers')
      .select('*')
      .applyTenantFilter('gt_vouchers')
      .eq('voucher_type', 'stockAdjustment')
      .order('voucher_date', ascending: false);

  final data = response as List<dynamic>;
  return data.map((json) => StockAdjustmentModel.fromJson(json)).toList();
});
