...
class ZoyarexDebitNoteForm extends ConsumerStatefulWidget {
  final dynamic note;

  const ZoyarexDebitNoteForm({Key? key, this.note}) : super(key: key);

  @override
  ConsumerState<ZoyarexDebitNoteForm> createState() => _ZoyarexDebitNoteFormState();
}

class _ZoyarexDebitNoteFormState extends ConsumerState<ZoyarexDebitNoteForm> {
  ...
  Future<void> _submitForm() async {
    ...
    try {
      ...
      if (widget.note == null) {
        await ZoyarexSupabase.client.from('debit_notes').insert(payload);
      } else {
        await ZoyarexSupabase.client.from('debit_notes').update(payload).eq('id', widget.note['id']);
      }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text(widget.note == null ? 'Create Debit Note' : 'Edit Note')),
      body: Form(
        key: _formKey,
        child: ListView(
