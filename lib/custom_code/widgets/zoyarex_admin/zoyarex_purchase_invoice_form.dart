...
class ZoyarexPurchaseInvoiceForm extends ConsumerStatefulWidget {
  final dynamic invoice;

  const ZoyarexPurchaseInvoiceForm({Key? key, this.invoice}) : super(key: key);

  @override
  ConsumerState<ZoyarexPurchaseInvoiceForm> createState() => _ZoyarexPurchaseInvoiceFormState();
}

class _ZoyarexPurchaseInvoiceFormState extends ConsumerState<ZoyarexPurchaseInvoiceForm> {
  ...
  void initState() {
    super.initState();
    _supplierCtrl = TextEditingController(text: widget.invoice?['supplier_name'] ?? '');
  ...
  Future<void> _submitForm() async {
    ...
    try {
      ...
      if (widget.invoice == null) {
        await ZoyarexSupabase.client.from('purchase_invoices').insert(payload);
      } else {
        await ZoyarexSupabase.client.from('purchase_invoices').update(payload).eq('id', widget.invoice['id']);
      }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text(widget.invoice == null ? 'Create Purchase Invoice' : 'Edit Invoice')),
      body: Form(
        key: _formKey,
        child: ListView(
