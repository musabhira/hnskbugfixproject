  ...
  Future<void> load({bool refresh = false}) async {
    ...
    try {
      final res = await Supabase.instance.client
          .from('pod_designs')
          .select(
              '*, pod_products(name, slug, category, mockup_image_url), profile(display_name, profile_image_url)')
          .eq('status', 'published')
...
final podMarketProvider =
    NotifierProvider.autoDispose<PodMarketNotifier, PodMarketState>(
...
class PodMarketplaceView extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    Widget bodyContent;
    if (s.isLoading && s.designs.isEmpty) {
      bodyContent = _skeleton();
    } else if (s.designs.isEmpty) {
      bodyContent = _empty(context);
    } else {
      bodyContent = NotificationListener<ScrollNotification>(
        onNotification: (n) {
    ...
    return Scaffold(
      ...
      body: RefreshIndicator(
        ...
        onRefresh: () =>
            ref.read(podMarketProvider.notifier).load(refresh: true),
        child: bodyContent,
      ),
    );
  }
        ...
        child: Row(children: [
          ...
          GestureDetector(
            onTap: () => Navigator.push(
              context,
              MaterialPageRoute(builder: (_) => const PodB2BPortal()),
            ),
            child: Container(
              padding:
  ...
  Widget _fabs(BuildContext context) => Column(
        ...
        children: [
          ...
          FloatingActionButton(
            ...
            mini: true,
            onPressed: () => _showMyDesigns(context),
            tooltip: 'My Designs',
  ...
  void _showMyDesigns(BuildContext context) {
    showModalBottomSheet(
      context: context,
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
      builder: (_) => const _MyDesignsSheet(),
    );
  }
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            ElevatedButton.icon(
              ...
              onPressed: () => Navigator.push(
                  context,
                  MaterialPageRoute(
                      builder: (_) => const PodCreatorStudio())),
              icon: const Icon(Icons.add, size: 18),
              label: Text('Create Your First Design',
                  style: GoogleFonts.inter(
                      fontWeight: FontWeight.bold, fontSize: 13)),
            ),
          ]),
  ...
  static Widget _skeleton() => Shimmer.fromColors(
...
class _PodDesignCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final imgUrl =
        (design['preview_image_url'] as String?)?.isNotEmpty == true
    ...
    final isB2B = design['b2b_available'] == true;
            ...
            child: Column(
                ...
                children: [
                  ...
                  Row(children: [
                    CircleAvatar(
                        radius: 9,
                        backgroundImage: artistImg != null
                            ? NetworkImage(artistImg)
                            : null,
...
// ─── My Designs Bottom Sheet ───────────────────────────────────────────────────

class _MyDesignsSheet extends StatefulWidget {
  const _MyDesignsSheet();

  @override
  State<_MyDesignsSheet> createState() => _MyDesignsSheetState();
}

class _MyDesignsSheetState extends State<_MyDesignsSheet> {
  ...
  Future<void> _delete(String designId) async {
    try {
      await Supabase.instance.client
          .from('pod_designs')
          .delete()
          .eq('id', designId);
      setState(() => _designs.removeWhere((d) => d['id'] == designId));
    } catch (e) {
      if (mounted) {
  ...
  Future<void> _updatePrice(
      String designId, double newPrice) async {
    try {
      await Supabase.instance.client
          .from('pod_designs')
          .update({'sale_price': newPrice}).eq('id', designId);
      final idx = _designs.indexWhere((d) => d['id'] == designId);
      if (idx != -1) {
        setState(() => _designs[idx]['sale_price'] = newPrice);
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
  ...
  void _showEditPrice(Map<String, dynamic> design) {
    final ctrl = TextEditingController(
        text: design['sale_price']?.toString() ?? '499');
    showDialog(
      context: context,
      builder: (_) => AlertDialog(
        ...
        content: TextField(
          ...
          decoration: InputDecoration(
            ...
            prefixText: '₹ ',
            prefixStyle: GoogleFonts.inter(color: _amber),
          ...
          ElevatedButton(
            ...
            onPressed: () {
              ...
              if (price != null && price > 0) {
                _updatePrice(design['id'] as String, price);
              }
              ...
              Row(children: [
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                      ...
                      borderRadius: BorderRadius.circular(12)),
                  child: Text('${_designs.length} designs',
                      style: GoogleFonts.inter(
                          color: _amber,
                          fontSize: 12,
                        ...
                        itemBuilder: (_, i) =>
                            _MyDesignTile(
                              ...
                              onDelete: () async {
                                ...
                                if (confirm == true) {
                                  _delete(_designs[i]['id'] as String);
                                }
                              },
                            ),
                      ),
  ...
  Widget _emptyState() => Center(
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          Icon(Icons.brush_outlined,
              size: 56, color: Colors.white.withValues(alpha: 0.06)),
          const SizedBox(height: 16),
          Text("You haven't published any designs",
              style: GoogleFonts.inter(
                  color: Colors.white38,
                  fontSize: 14,
                  fontWeight: FontWeight.w500)),
...
class _MyDesignTile extends StatelessWidget {
  final Map<String, dynamic> design;
  ...
  Widget build(BuildContext context) {
    ...
    final price = design['sale_price'];
    final status = design['status'] as String? ?? 'pending';
    final product = design['pod_products'] as Map<String, dynamic>?;
    final productName = product?['name'] as String? ?? '';
    final isB2B = design['b2b_available'] == true;
...
class _PodProductDetailPageState extends State<PodProductDetailPage> {
  ...
  Widget _successDialog() => AlertDialog(
        ...
        content: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          const SizedBox(height: 8),
          Text(
              'The artist will contact you on WhatsApp to confirm and ship.',
  ...
  Widget build(BuildContext context) {
    ...
    final imgUrl =
        (design['preview_image_url'] as String?)?.isNotEmpty == true
            ...
            : design['design_image_url'] as String? ?? '';
    final slug =
        (design['pod_products'] as Map?)?['slug'] as String? ?? '';
    final profile = design['profile'] as Map<String, dynamic>?;
    final isB2B = design['b2b_available'] == true;
          ...
          if (isB2B)
            TextButton.icon(
              onPressed: () => Navigator.push(
                  ...
                  MaterialPageRoute(
                      builder: (_) =>
                          PodB2BPortal(preSelectedDesign: design))),
        ...
        child: Column(children: [
          Container(
            ...
            child: Pod2DPreviewWidget(
              mockupImageUrl:
                  (design['pod_products'] as Map?)?['mockup_image_url']
                      as String?,
              designImageUrl: imgUrl.isNotEmpty ? imgUrl : null,
              productSlug: slug,
              width: double.infinity,
              height: 280,
              posX: design['design_pos_x'] != null
                  ? (design['design_pos_x'] as num).toDouble()
                  : 0.5,
              posY: design['design_pos_y'] != null
                  ? (design['design_pos_y'] as num).toDouble()
                  : 0.4,
              scale: design['design_scale'] != null
                  ? (design['design_scale'] as num).toDouble()
                  : 0.45,
              rot: design['design_rot'] != null
                  ? (design['design_rot'] as num).toDouble()
                  : 0.0,
              isEditing: false,
            ),
          ),
          Padding(
            ...
            child: Column(
                ...
                children: [
                    ...
                    Row(children: [
                      CircleAvatar(
                          ...
                          backgroundImage: (profile['profile_image_url']
                              ...
                              : null,
                          backgroundColor: const Color(0xFF2A2A2A),
                          child: (profile['profile_image_url'] as String?) ==
                                  null
                              ? const Icon(Icons.person,
                      ...
                      Text(
                          'by ${profile['display_name'] ?? 'Artist'}',
                          style: GoogleFonts.inter(
                              color: Colors.white38, fontSize: 12)),
                      if (isB2B) ...[
                        const SizedBox(width: 8),
                  ...
                  _field(_phoneCtrl, 'Phone *', '10-digit number',
                  ...
                  Row(children: [
                    Expanded(
                        child: _field(_cityCtrl, 'City', 'City')),
                    const SizedBox(width: 10),
                    Expanded(
                        child: _field(
                            _pincodeCtrl, 'Pincode', '6 digits',
                  ...
                  Container(
                    ...
                    child: Column(children: [
                          ...
                          child: _saving
                              ? const SizedBox(
                                  ...
                                  child: CircularProgressIndicator(
                                      color: Colors.black,
                                      strokeWidth: 2))
                              : Text('Place Order (Manual Payment)',
                                  style: GoogleFonts.inter(
                                      fontWeight: FontWeight.bold,
                                      fontSize: 14)),
                        ),
                      ...
                      const SizedBox(height: 8),
                      Text(
                          '💬 The artist will contact you on WhatsApp to confirm payment.',
                          textAlign: TextAlign.center,
                          style: GoogleFonts.inter(
